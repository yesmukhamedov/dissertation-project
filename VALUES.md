...
### 1.1 Метрики по армам — EyePACS, n = 35 126, 5-fold patient-level CV
...
### 1.4 Bootstrap по конфигам — wF1, 1000 resamples
...
### 1.8 Клинические in-domain — referable DR (grade ≥ 2)
...
| Config | best-эпохи по фолдам | train_loss (best) | val_loss (best) | loss-gap |
|--------|----------------------|------------------:|----------------:|---------:|
...
### 2a.3 Расширенный диапазон, clip_factor = 0.5
...
n = 35 126 · 5 фолдов · EfficientNet-B3

### 2d.1 Кумулятивная аблация — 8 уровней

| Level | Стадии | Weighted F1 | ROC-AUC | Cohen κ | Accuracy | σ_fold | \|Δⱼ\| | 2 · σ_fold |
|-------|--------|------------:|--------:|--------:|---------:|-------:|-------:|-----------:|
...
### 2d.2 Пофолдовые wF1 по уровням
...
### 2d.3 Качество изображения по уровням — n = 100

| Level | mean CNR | mean Entropy (бит) | mean SSIM |
|-------|---------:|-------------------:|----------:|
...
### 3.1 MMD по признакам предпоследнего слоя
...
### 3.2 KL по канальным гистограммам
...
Stage 7 — статистика исходного домена.
...
| Арм | wF1 | ROC-AUC | Cohen κ | Accuracy | macro-F1 |
|-----|----:|--------:|--------:|---------:|---------:|
...
### 5.1 ALO — IDRiD, n = 54, τ = 0.5, бэкбон E3

| Тип | n | ALO (C) | ALO (D) | Δ ALO | 95% CI (Δ) | p (Wilcoxon, 1-стор.) |
|-----|--:|--------:|--------:|------:|------------|----------------------:|
...
| Тип | n | IoU (C) | IoU (D) | Δ IoU | 95% CI (Δ) | p (Wilcoxon, 1-стор.) |
|-----|--:|--------:|--------:|------:|------------|----------------------:|
...
### 5.3 Направление по изображениям
...
| Величина | Значение |
|----------|---------:|
| f₀ (ALO = 0 в обеих арках) | 6 / 54 = 0.1111 |

### 5.5 Устойчивость к порогу
...
### 5.6 Классификация арок — EfficientNet-B4, fold 0

| Арм | best эпоха | Weighted F1 | ROC-AUC | Cohen κ | Accuracy |
|-----|-----------:|------------:|--------:|--------:|---------:|
...
Размеры групп: kowa_idrid 413 · mixed_ddr 1 200 · mixed_odir5k 950 · topcon_messidor2 1 744 · mixed_rfmid 640

### 6.1 Weighted F1 по группам

| Группа | wF1 (C) | wF1 (D) | Δ | 95% CI (Δ) |
|--------|--------:|--------:|--:|------------|
...
### 6.2 ROC-AUC по группам

| Группа | AUC (C) | AUC (D) | Δ | 95% CI (Δ) |
|--------|--------:|--------:|--:|------------|
...
### 6.5 Referable-DR по группам

| Группа | Sens (C) | Spec (C) | PPV (C) | NPV (C) | Sens (D) | Spec (D) | PPV (D) | NPV (D) |
|--------|---------:|---------:|--------:|--------:|---------:|---------:|--------:|--------:|
...
### 6.6 Per-class F1 по группам — Config C
...
### 6.7 Per-class F1 по группам — Config D
...
### 6.8 Разброс по группам

| Величина | C | D | Δ (D − C) | 95% CI (Δ) |
|----------|--:|--:|----------:|------------|
...
### 7.1 Внешние клинические наборы
...
Корпус SSL: немаркированный EyePACS «test» split, n = 53 576 · протокол BYOL · 4 канала

### A1.1 Этап 1 — from-scratch SSL

| Метод | эпохи | κ | passed |
|-------|------:|--:|--------|
...
### A7.1 Вычислительные бенчмарки — RTX 3060 12 GB, вход 512×512, fp32

| Config | Params | GFLOPs/изобр. | Latency bs=1 (мс) | Latency bs=16 (мс/изобр.) | Throughput bs=16 (изобр./с) | VRAM инференс bs=16 (МиБ) | VRAM train-step bs=16 (МиБ) |
|--------|-------:|--------------:|------------------:|--------------------------:|----------------------------:|--------------------------:|----------------------------:|
