...
|-----|------------:|--------:|--------:|---------:|
| C — baseline (3ch) | 0.5134 ± 0.0450 | 0.4876 ± 0.0440 | 0.7417 ± 0.0380 | 0.5231 ± 0.0410 |
| D — full pipeline (4ch) | **0.5932 ± 0.0400** | **0.6121 ± 0.0438** | **0.7899 ± 0.0320** | **0.5932 ± 0.0370** |
...
> ⚠️ **The unpaired intervals overlap** (C up to 0.6007, D from 0.5338) — at n = 60 the per-instance
...
The pipeline is above baseline **in 4 of the 5 folds** (margins +0.1262, +0.0952, +0.0906, +0.0287);
on **fold 3** it is marginally lower (0.6232 against 0.6289, −0.0057). Fold 3 is baseline's strongest
fold — the inversion is a single-fold fluctuation an order of magnitude smaller than the typical
margin and well inside the between-fold std (0.031–0.038), not a systematic exception. The mean
advantage (+0.0670 across folds) holds, and the per-image paired test on the clinical hold-out
(§E7.3) is the primary evidence.

⚠️ Note that the previous revision also had 4/5, but the failing fold was **fold 2**. Which fold
inverts is not stable across runs — report "4 of 5 folds, one marginal inversion", without naming the
fold as if it were a property of the data.
...
On small clinical data the pipeline delivers **+0.080 weighted-F1**, **+0.125 κ** and **+0.048 AUC**
with intervals excluding zero. The κ gain (+0.125) is over 1.5× the wF1 gain — the same picture as in
...
time, the gain here (+0.080) is comparable to the gain on full EyePACS (+0.0655, `TAB-4.2`), i.e. the
pipeline's advantage **does not vanish** when moving to large data and **is not** an artifact of the
small sample.

## Caveats

- The absolute level (wF1 ≈ 0.51–0.59) is noticeably below in-domain EyePACS — as expected for
