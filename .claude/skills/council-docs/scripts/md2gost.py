...
def _body(doc: Document, text: str, *, center: bool = False, right: bool = False):
    ...
    `right` is the approval grif of the extended-meeting protocol — «УТВЕРЖДАЮ»
    over the pro-rector's post, organisation, signature rule and date, set flush
    right and unindented in the top-right corner of the first page.
    """
    p = doc.add_paragraph()
    if center or right:
        p.alignment = (WD_ALIGN_PARAGRAPH.CENTER if center
...
def render_into(
    ...
    *,
    ...
    # `<!-- right -->` mirrors it for the approval grif of the extended-meeting
    # protocol, which the samples set flush right at the top of the first page.
    pending_right = [False]
    ...
    for raw in lines:
        ...
        # Comment directives. The first two exist for the list of scientific papers:
