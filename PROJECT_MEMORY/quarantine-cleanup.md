...
description: Архивные/дублирующие файлы вынесены из проекта в D:\удаленные (не удалены); что проверено безопасным, что осознанно оставлено и какие крупные кандидаты ждут решения
metadata:
  type: project
---

2026-08-12 из `D:\dissertation-project` вынесены (НЕ удалены) архивные, дублирующие и
...
⚠️ `D:\удаленные` лежит **вне** `dissertation-project`, поэтому на внешний диск не переезжает —
при переносе проекта карантин надо копировать отдельно или разобрать. См. [[eyepacs-local-dataset]].
