...
- [Demo stack](PROJECT_MEMORY/demo-stack.md) — **SETTLED launch protocol, act don't ask**: «локально» → `demo/start-demo.ps1`; «публично» → `demo/start-pages-demo.ps1` → permanent **https://dr-classification.pages.dev** (Cloudflare Pages + quick tunnel, $0, decided 2026-08-03); password always on from gitignored `demo/.demo-password`; backend `auto` = native venv, no WSL on this box. Also: `server/requirements.txt` misses pandas/sklearn; `web/build/` goes stale silently; stale-PATH breaks spawned npm; Pages returns 200+index.html for missing assets. Protocol copy `demo/CLAUDE.md`, human copy `demo/RUNBOOK.md`
