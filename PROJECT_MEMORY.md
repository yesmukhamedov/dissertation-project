...
- [Demo stack](PROJECT_MEMORY/demo-stack.md) — local launch = `demo/start-demo.ps1`/`.bat` (one-shot, default); backend runs **native-Windows venv OR WSL** (`-Backend auto`), WSL not required; `server/requirements.txt` misses pandas/sklearn/matplotlib; `web/build/` goes stale silently; Cloudflare tunnels for the real model; human copy `demo/RUNBOOK.md`
