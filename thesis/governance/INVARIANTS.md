...
**v7.1.0 Amendment Summary (vs v7.0.0):** **SB-4.1 is amended** to record that a working demonstrator of the system architecture is deployed and performs inference on submitted images. **Motivation:** the clause asserted that no prototype implementation was available, which was true when it was written and is no longer true; a binding constraint that misdescribes the state of the work cannot bind correctly. The amendment is bounded on both sides — the demonstrator establishes realizability and operating behaviour, and is evidence for no diagnostic claim; **SB-4.2** (compliance framing is a design specification, not a certified status) and **SB-4.3** (applicability to Kazakhstan is bounded by the absence of field testing) are unchanged and continue to bind. Section IX is brought into line. **Also in this pass:** the volume is restructured from six chapters to four (`outline/MASTER_OUTLINE.md` v8.0.0, `outline/REWRITE_MAP.md`), so the chapter and section references carried in the clause text are renumbered — Chapter 6 → Chapter 4 (SB-4.1, Section IX), Chapter 2 Section 2.4 → Chapter 1 section 1.1 (SB-1.5), Section 4.1.3 → section 3.1 (Section IX). Renumbering changes no binding. **Relaxes one scope boundary to match fact; reverses no hypothesis, no forbidden claim and no non-claim → MINOR bump** per VERSIONING_POLICY §4. Hypotheses H-1 through H-7, all forbidden claims, all non-claims, the composite independent variable and CFC-2.8 are **unchanged**.
...
- SB-4.1 [amended v7.1.0] The system architecture described in Chapter 4 is a design contribution (LC-2025-Yesmukhamedov-01), and a **working demonstrator of it is deployed and performs inference on submitted images**: it runs the preprocessing pipeline and the trained classifier end to end, produces attention maps, and keeps a per-case record including the reviewing ophthalmologist's verdict. What the demonstrator establishes is that the design is realizable and what it does in operation; it establishes nothing about clinical performance in service. **No clinical deployment testing has been conducted**, and no result of the demonstrator is evidence for any diagnostic claim — every such claim rests on the experiments of Chapter 3 and on nothing else. The prior form of this clause ("no prototype implementation … available") was written before the demonstrator existed and had become false.
...
The system architecture described in Chapter 4 (LC-2025-Yesmukhamedov-01) is realized as a deployed demonstrator (SB-4.1) but has not been field-tested in a clinical setting. The deployment-oriented statements that the demonstrator does not itself realize — PACS integration, EHR interoperability, certified GDPR/HIPAA compliance, telemedicine operation — remain design specifications. Their operational realization in Kazakhstan's healthcare infrastructure is subject to infrastructure prerequisites acknowledged in the source (LC-2025-Yesmukhamedov-01, p. 90): investments in diagnostic equipment, adaptation of algorithms to local data, national standards development, and specialist training.
