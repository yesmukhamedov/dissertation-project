...
> Draft generated per `prompts/writing-session-system-prompt.md` v6.0.0 · Brief: `briefs/F-brief.md` · Binding reference: INVARIANTS.md v7.0.0 · Sources: the consolidated device-shift result tables of `results/` (per-group weighted F1, retention ratio, ROC-AUC, κ, referable AUC, between-group dispersion; per-class F1 by group and per-group class sizes), with §4.8 for the established reading and §5.4 for the normalisation argument. **Every figure is transcribed, not recomputed.** Figures: none — this appendix is tabular throughout.
...
**Three governing conditions stated before the first table** — ✅ The two coinciding groupings (*"not independent evidence"*), the three aggregating groupings (*"a per-group figure is not a per-device figure"*), and the single fold (*"the dispersion reported… is between groups, not between folds"*). A fourth, NC-16, is added in the same passage.

**NC-16 at the opening and the close** — ✅ Opening: *"none of what follows constitutes device certification, regulatory compliance, or a claim of device-agnostic deployment readiness."* Close: *"None of what this appendix contains supports a claim of device compatibility, device certification or regulatory compliance."*

**Transcription only, no new number** — ✅ Every figure in Tables F.1–F.8 is transcribed from the consolidated result tables. No dispersion statistic, span or ratio is recomputed. The only arithmetic remarks — the factor-of-2.4 and factor-of-3.1 reductions, and the twenty-five-cell count — restate §4.8.

**SB-2.1 / class sizes before per-class figures** — ✅ §F.1 gives per-group class sizes with the instruction to read §F.5 against them and the warning that DR3 and DR4 rest on tens of images.

**SB-2.3 (equipment and taxonomy heterogeneity)** — ✅ Carried by the second governing condition and by the closing bound on camera models not represented.

**SB-1.8** — ✅ Satisfied by the NC-16 statements; device-shift results are empirical observations only.

**EH-1 / EH-2** — ✅ Weighted F1 leads; per-class figures are presented as decomposition and establish nothing on their own.

**CFC-2.8** — ✅ Satisfied. Every comparison is between configurations C and D; no observation is attributed to preprocessing alone.

**NC-17** — ✅ Satisfied (absent). No configuration or grouping is described as optimal.

**CFC-2.1 / CFC-2.2 / CFC-2.4 / CFC-2.5** — ✅ Satisfied (absent). The closing paragraph bounds extension to unrepresented camera models; no published system is compared against; no clinical or perfect-performance claim.

**Retention-ratio artefact bounded as descriptive** — ✅ §F.8: *"The observation is descriptive. It explains the shape of a column in Table F.2; it does not rehabilitate any result, and no claim in this dissertation is strengthened by it."*

**F.9 present with its consequence** — ✅ The absence is named, its consequence for within-group error composition is given, and the cost of closing it is stated (an export, not new training).

**SIR-1 / SIR-3** — ✅ No reading exceeds §4.8's. Every metric carries its group, its arm and its class context; no figure is compared across corpora without that context.

**Rule 16** — ✅ Satisfied, and this was the live risk since the source tables carry run dates, source-file section pointers and an account of which rows were re-synchronised between revisions. **None crossed over.** No run date, no artifact or source-file path, no checkpoint identifier, and no revision narrative appears. The single-fold statement is a methodological fact required to judge the dispersion figure, not process metadata.

### Word count

Prose ≈ 940 words excluding tables; eight tables.
