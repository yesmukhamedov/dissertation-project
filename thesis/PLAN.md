...
- **NEW-1 traceability — OPEN, and the only remaining item that cannot be closed from `thesis/`.**
  Checked on disk 2026-08-13 rather than taken from the flag, and the picture is more precise than
  "artifacts missing":

  **The chain stops at `VALUES.md`, and it is intact up to that point.** Every signature value the
  dissertation states was traced to it and found: the four Config wF1 values and all six Exp-1 pairwise
  differences, the CLAHE optimum and the flat-field σ, both generalization ratios, the ALO effect
  bounds, H-7's +0.0689 / +0.0541 and the 0.0041 margin, Exp-6's four spread figures, E-7's three
  differences, and the H-3 distance-reduction bounds. **Nothing in the text is unsourced.**

  **What is missing is the artifact chain behind `VALUES.md`** — a hand-transported summary is not a
  primary output file. `experiments/outputs/` holds a *different, earlier* run, and the disagreement is
  not a rounding matter: its Exp-5 artifact records `h7_supported: false` under the retired
  specification, and its absolute figures put IDRiD at wF1 0.5388 integrated against 0.6154 baseline —
  **the opposite sign** to the +0.0689 the dissertation reports. The Exp-2 artifact carries a
  three-fold baseline of 0.7611 where the text has a five-fold L0 of 0.7538. The specification changed
  between the two runs, so this is not evidence against the current result; it *is* the reason the old
  artifacts can never serve as the corroboration NEW-1 asks for.

  **This was already stated honestly in `results/INTEGRITY_NOTE.md`**, which sets the standing rule —
  take numbers from `results/tables/` and `STATUS.md`, and do **not** cross-check against
  `results/data/*.json` or `experiments/outputs/`, which belong to a different run.

  **To close it, the candidate must produce the 2026-08-03 run-2 artifacts** from wherever that run
  executed. No amount of work inside this repository will close it.

  **One thing verified while checking, and it is good news:** the contaminated demo-derived plates
  (`fig8_training_curves.png`, `fig9_confusion_matrix.png`) that `INTEGRITY_NOTE.md` §2 names are
  **not referenced by either manuscript**. Of the nine `figures_mine` plates the text does use, the
  only one carrying measurements is FIG-4.17, which is generated by parsing `results/tables/`. **No
  hand-transcribed number reaches a figure in the dissertation.**
