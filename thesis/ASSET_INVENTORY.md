...
|----|------|----------------------|----------|--------|-----------|--------|
| FIG-1.1 | figure | Representative fundus images across DR grades 0–4 (clinical grading context) | §1.1.1 | **IDRiD** graded set (516 images) | `defense/figures/figures_mine/fig1_1_dr_grades_idrid.png`, built by `_make_idrid_figures.py` | ✅ AVAILABLE — **corpus corrected 2026-08-12**: the §1.1.1 placeholder named `fig1_1_dr_grades_eyepacs.png`, which does not exist. Per `figures_mine/README.md`, dataset-illustration figures were deliberately rebuilt from IDRiD, and the plate carries "(IDRiD)" in its own title — so the EyePACS caption contradicted the image. Path and caption now both say IDRiD (SIR-2). The EyePACS generator `_make_dataset_montages.py` remains on disk but its output was never kept. |
...
| FIG-3.8 | figure | Stage 6 — Augmentation (affine/ColorJitter/Gaussian-noise/JPEG) | §3.1.3 | defense / demo | `defense/presentation/assets/preprocessing/19_aug_rotation/stage6_augmentation.png` (six identical copies across `19_…`–`24_…`) | ✅ **REGENERATED 2026-08-12** by `defense/presentation/scripts/render_stage6_card.py` from `demo/web/public/diagrams/03_preprocessing_stages_detailed.svg`. The old render showed the retired **PCA colour jitter** and a **"horizontal re-flip"** step the implementation does not perform. A layout collision in the re-specified source (a 4th PARAMETERS line at the panel grid's OUTPUT y) was fixed in the same pass, and the full sheet re-rendered. Parameters verified against `config.py`. ⚠️ The containing folder is still named `23_aug_pca_color` — a path name only, no longer describing its contents. |
...
| FIG-4.17 | figure | H-3 domain-distance reduction across the six target domains, both measures | §4.4.2 | H-3 | `defense/figures/figures_mine/fig4_17_domain_distance.png`, built by `_make_fig4_17.py` | ✅ **RENDERED 2026-08-12** — the last `ASSET TO BE CREATED` in the manuscript. The script **parses** `results/tables/H-3_domain_distance.md` rather than transcribing it, so the figure cannot drift from the table above it. Three panels: distance per arm (ordering visibly preserved), Δd against MCID_d = 0 (all six intervals clear of zero), KL reduction against its 34–38 % band. Caption carries the model-dependence caveat and the secondary status of KL; **no panel pairs Δd with transfer gain** (ρ ≈ 0.49 — direction only). |
...
| DIA-6.3 | diagram | System-architecture structural views — component, deployment, sequence, data | §6.1.2 / App C | App C | `chapters/08-appendices/drafts/C-draft.md` §C.1–§C.4, Mermaid source | ✅ **DISCHARGED** — Appendix C supplies four views + traceability Table C.1. The earlier description (component/sequence/class/activity/ER) was a promise, not a delivery; §6.1.2 was corrected 2026-08-12 to name the four views actually supplied. **Conversion requirement: the four Mermaid fences must render at conversion.** |
...
| APP-B | figure-set | Supplementary confusion matrices & training curves | App B | Exp 1–7 | `chapters/08-appendices/drafts/B-draft.md` — 13 tables | ✅ **WRITTEN & APPROVED** — transcription verified mechanically (168 distinct 3–4 dp values + 184 confusion-matrix integers, none absent from `results/`, nothing recomputed). §B.8 states three absences with a reason each: per-epoch trajectories not retained, per-class ROC/PR need prediction dumps that were not saved, per-group confusion matrices not recorded. |
| APP-C | diagram-set | System architecture structural views | App C | — | `chapters/08-appendices/drafts/C-draft.md` | ✅ **WRITTEN & APPROVED** — discharges DIA-6.3; four Mermaid views + module → FR → NFR traceability table |
...
| APP-E | figure-set | Grad-CAM visualization gallery | App E | Exp 4 | `chapters/08-appendices/drafts/E-draft.md`; plates at `experiments/outputs/exp4/gradcam_maskset/` | ✅ **WRITTEN & APPROVED** — the **complete** 54-plate annotated subset, no selection, floor cases retained; NC-14 before the first plate; **IDRiD only — G-3 stated as an absence**, the clinical (KZ) overlays were never produced |
| APP-F | table-set | Device domain-shift supplementary per-camera tables | App F | Exp 6 | `chapters/08-appendices/drafts/F-draft.md` — 8 tables | ✅ **WRITTEN & APPROVED** — 159 distinct values verified against source; three qualifications stated *before* the tables; §F.8 explains the retention-ratio artefact as an instance of the §5.4 normalisation defect, bounded as descriptive; §F.9 records the missing per-group confusion matrices |
